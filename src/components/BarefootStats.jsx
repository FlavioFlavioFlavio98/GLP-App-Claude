import { computeBarefootStats } from '../lib/bodyStats'
...
export default function BarefootStats({ barefootLog, actions }) {
  const hasAny = Object.keys(barefootLog || {}).length > 0
  if (!hasAny) return null

  const stats = computeBarefootStats(barefootLog)
  const todayStr = toDateString(new Date())
  const todaySessions = (barefootLog?.[todayStr] || []).slice().sort((a, b) => (a.time || '').localeCompare(b.time || ''))
            ...
            <div key={s.id} style={{
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  await actions.deleteBarefootSession(todayStr, s.id)
                }}
              >
