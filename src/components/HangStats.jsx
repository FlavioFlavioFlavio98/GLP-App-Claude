import { computeHangStats } from '../lib/bodyStats'
...
export default function HangStats({ hangLog, actions }) {
  const hasAny = Object.keys(hangLog || {}).length > 0
  if (!hasAny) return null

  const stats = computeHangStats(hangLog)
  const todayStr = toDateString(new Date())
  const todaySessions = (hangLog?.[todayStr] || []).slice().sort((a, b) => (a.time || '').localeCompare(b.time || ''))
            ...
            <div key={s.id} style={{
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  await actions.deleteHangSession(todayStr, s.id)
                }}
              >
