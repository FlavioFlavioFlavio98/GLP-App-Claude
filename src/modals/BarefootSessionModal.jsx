...
import { getBarefootRate } from '../lib/bodyStats'

export default function BarefootSessionModal() {
  ...
  if (modal !== 'barefoot') return null
  if (authUserId !== 'flavio') return null

  const rate = getBarefootRate()
  ...
  async function handleAdd() {
    setSaving(true)
    await actions.addBarefootSession(duration, sessionDate)
    setSaving(false)
    actions.closeModal()
  }
    ...
    >
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 16, marginBottom: 14 }}>
          <button onClick={() => changeDuration(-15)} style={btnStyle}>−15</button>
          <button onClick={() => changeDuration(-5)} style={{ ...btnStyle, width: 52, height: 52, fontSize: '1.1em' }}>−5</button>
