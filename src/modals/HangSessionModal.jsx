...
import { getHangRate } from '../lib/bodyStats'

export default function HangSessionModal() {
  ...
  if (modal !== 'hang') return null
  if (authUserId !== 'flavio') return null

  const rate = getHangRate()
  const pts = Math.round(duration * rate * 100) / 100

  function changeDuration(delta) {
    setDuration(prev => Math.max(0.5, Math.round((prev + delta) * 10) / 10))
  }

  async function handleAdd() {
    setSaving(true)
    await actions.addHangSession(duration, sessionDate)
    setSaving(false)
    actions.closeModal()
  }
    ...
    >
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 16, marginBottom: 14 }}>
          <button onClick={() => changeDuration(-1)} style={btnStyle}>−1</button>
          <button onClick={() => changeDuration(-0.5)} style={{ ...btnStyle, width: 52, height: 52, fontSize: '1em' }}>−.5</button>
...
const btnStyle = {
  ...
  color: 'var(--text)', fontSize: '0.85em', fontWeight: 700,
